...
<!-- EN: Data access capability for e-commerce Starter, supporting dual-track JPA and MyBatis implementation -->
<!-- CN: 电商 Starter 的数据访问能力，支持 JPA 和 MyBatis 双轨实现 -->
...
**中文说明**：
本能力定义了电商 Starter 的数据访问层规范，核心是实现 JPA 和 MyBatis 的双轨支持。通过条件装配自动检测用户环境，无需额外配置即可使用。通过抽象接口层隔离业务逻辑和数据访问实现，保证业务代码的框架无关性。

**EN Description**:
This capability defines the data access layer specification for e-commerce Starter, with dual-track support for JPA and MyBatis as the core. Through conditional assembly, it automatically detects the user environment without additional configuration. Through an abstract interface layer, it isolates business logic from data access implementation, ensuring framework-agnostic business code.
